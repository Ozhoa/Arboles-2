...
public class Lienzo extends JPanel implements MouseListener {
    ...
    public void mouseClicked(MouseEvent e) {
        if(e.getButton()==MouseEvent.BUTTON1){
            this.vertices.add(new Vertice(e.getX(),e.getY()));
            repaint();
        }

        if(e.getButton()==MouseEvent.BUTTON3){
            for(Vertice vertice:vertices){
                if(new Rectangle(vertice.getX()-Vertice.diametro/2,
                        vertice.getY()-Vertice.diametro/2,
                        Vertice.diametro,Vertice.diametro).contains(e.getPoint()))
                {
                   if(p1==null){
